...
V1.3.3.20170612
1、移植msOS_msPLC-100的V1.3.3.20170302版本，保持版本同步
2、编译器升级为MDK5
3、修改device_key.c中对swd的配置，改成GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);，支持JLINK调试

V1.4.0.20171201
1、os.c的任务切换代码修改的更加紧凑，提升内核切换速度。

V1.4.1.20180317
1、device_key.c的检测方式修改，提升按键可靠性

V1.4.2.20180909
1、增加了PID库，用于温控等场合。
2、GUI库的TextBox结构体，增加了函数执行成员，用于文本值修改后，可以直接执行函数，应用于PWM等需要执行函数的场合。